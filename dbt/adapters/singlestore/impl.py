...
class SingleStoreAdapter(SQLAdapter):
    ...
    def get_columns_in_relation(self, relation: Relation) -> List[SingleStoreColumn]:
    ...
    def _get_one_catalog(
        ...
        if len(schemas) != 1:
            dbt.exceptions.raise_compiler_error(
                f'Expected only one schema in singlestore _get_one_catalog, found '
                f'{schemas}'
            )
    ...
    def get_rows_different_sql(
        self,
        relation_a: SingleStoreRelation,
        relation_b: SingleStoreRelation,
        ...
        # SingleStore doesn't have an EXCEPT or MINUS operator, so we need to simulate it
        COLUMNS_EQUAL_SQL = '''
        WITH
        a_except_b as (
            SELECT
